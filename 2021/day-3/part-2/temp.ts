...
function computeRatings(input: string[]): computeRatingsResponse {
	const oxygenGeneratorBitIdentifier: BitIdentifier = (
		numZeroes: number,
		inputLength: number,
	): string => {
		return numZeroes > inputLength / 2 ? "0" : "1";
	};
	const carbonDioxideBitIdentifier: BitIdentifier = (
		numZeroes,
		inputLength,
	) => {
		return numZeroes <= inputLength / 2 ? "0" : "1";
	...
	return {
		...
		carbonDioxideScrubberRating: computeRating(
			input,
			carbonDioxideBitIdentifier,
		),
	};
}
...
let { oxygenGeneratorRating, carbonDioxideScrubberRating } = computeRatings(
	input,
);
console.log(
	`Part 2 Result = ${oxygenGeneratorRating * carbonDioxideScrubberRating}`,
);
export {};
