  ...
  const monkeys: Monkey[] = input
    ...
    .map((g, i) => {
      ...
      const operation = (worry: number) => {
        const [op, amount] = g.at(1)!.replace("Operation: new = old ", "").split(" ");
      ...
      const condition = (worry: number) => {
        const trueMonkey = parseInt(g.at(3)!.replace("If true: throw to monkey ", ""));
        const falseMonkey = parseInt(g.at(4)!.replace("If false: throw to monkey ", ""));
